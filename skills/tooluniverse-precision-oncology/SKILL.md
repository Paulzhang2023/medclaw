...
_Interpretation: KRAS-mutant pancreatic cancer lines are highly dependent on KRAS - validates targeting strategy._

_Source: DepMap via `DepMap_get_gene_dependencies`_
```
...
_Source: Human Protein Atlas via `HPA_get_comparative_expression_by_gene_and_cellline`_
```
...
_Source: GDC via `GDC_get_mutation_frequency`, `GDC_get_cnv_data`_
...
_Effect score <-0.5 = strongly essential for cell survival_
_Source: DepMap via `DepMap_get_gene_dependencies`_

_Combined Sources: CIViC, ClinVar, COSMIC, GDC/TCGA, DepMap_
```
...
_Source: PubMed, BioRxiv, MedRxiv, OpenAlex_
```
